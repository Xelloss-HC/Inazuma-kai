...
var Client = {
		...
		this.change(function(mutations) {
			mutations.forEach(function(mutation) {
				...
				if (mutation.attributeName === 'class' &&
					mutation.target === document.querySelector('.pop-usual')) {
				...
				if (mutation.type === 'childList' &&
					...
					mutation.target === document.querySelector('#pop')) {
					if ($('.btn-control').is(':visible')) {
						this.click('.btn-control');
					}
				}
				if (mutation.attributeName === 'style' &&
					mutation.target === document.querySelector('.btn-control')) {
	...
	sendCommand: function(command, callback) {
		chrome.extension.sendRequest(command, callback || function() {});
	},
	sleep: function(sec) {
		return new Promise(function(resolve) {
			setTimeout(function() {
	...
	isFarmingQuest: function() {
		return window.localStorage.getItem('quest-id') !== '' &&
